...
const QuizCard = ({ question }: { question: questionType[] }) => {
  ...
  >("NOT FILL");
  const [answers, setAnswers] = useState<(string | undefined | string)[]>([]);
    ...
    <div className="grid grid-cols-2 grid-rows-3 w-full">
      {stop ? (
        <>
          ...
          <div className="question col-span-1 row-span-1">
            <h1 className="flex items-center justify-center text-4xl">
              {data?.question}
            </h1>
          </div>
          <div className="options col-span-1 gap-4 w-full p-9 row-span-1">
          ...
          <div className="main">
            <button
              onClick={updateValue}
              className="bg-green-700 px-3 py-2 text-xl font-semibold rounded-lg"
            >
              Next
            </button>
          </div>
